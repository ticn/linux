#!/bin/bash
# 一键管理 IPv6 SIT 隧道服务，支持安装、卸载、状态、自动检测和修复
...
# 删除旧路由避免冲突
ip -6 route del ::/0 dev sit1 2>/dev/null || true
...
EOF
    ...
    systemctl enable --now "$SERVICE_NAME" "$TIMER_NAME"

    echo "安装完成！服务已启动，自动检测每 5 分钟运行一次。"
}

function uninstall() {
    echo "=== 卸载 IPv6 SIT 隧道服务及自动检测 ==="

    systemctl stop "$TIMER_NAME" "$CHECK_SERVICE_NAME" "$SERVICE_NAME" 2>/dev/null || true
    systemctl disable "$TIMER_NAME" "$CHECK_SERVICE_NAME" "$SERVICE_NAME" 2>/dev/null || true

    rm -f "$SERVICE_FILE" "$CHECK_SERVICE_FILE" "$TIMER_FILE" "$SCRIPT_FILE" "$CHECK_SCRIPT_FILE" "$CONFIG_FILE" "$LOGROTATE_FILE" "$LOG_FILE"
...
function status() {
    echo "=== IPv6 SIT 隧道状态 ==="
    ip link show sit0 2>/dev/null || echo "sit0 接口不存在"
    ip link show sit1 2>/dev/null || echo "sit1 接口不存在"
    ip -6 addr show dev sit1 2>/dev/null || echo "sit1 没有 IPv6 地址"
    ip -6 route show dev sit1 2>/dev/null || echo "sit1 无 IPv6 路由"
    ...
    echo
    echo "自动检测服务状态:"
    systemctl status "$TIMER_NAME" "$CHECK_SERVICE_NAME" "$SERVICE_NAME" --no-pager || true
}
